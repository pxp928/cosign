...
func AttestCmd(ctx context.Context, ko options.KeyOpts, regOpts options.RegistryOptions, imageRef string, certPath string, certChainPath string,
	...
	signedPayload, err := getSignedPayload(ctx, wrapped, predicate, predicatePath, predicateType, h.Hex, digest.Repository.String())
	if err != nil {
	...
	if sign.ShouldUploadToTlog(ctx, digest, force, ko.RekorURL) {
		bundle, err := attest(ctx, sv, signedPayload, ko.RekorURL)
		if err != nil {
			return err
		}
